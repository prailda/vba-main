...
topic:
  - Codex
  - Codebase
description: "This cookbook shows how to use OpenAI's Codex CLI to modernize a legacy repository in a way that is:\r\r Understandable to new engineers\r; Auditable for architects and risk teams\r; Repeatable as a pattern across other systems"
...
---
## [Introduction](https://cookbook.openai.com/examples/codex/code_modernization#introduction)
...
## [High Level Overview](https://cookbook.openai.com/examples/codex/code_modernization#high-level-overview)
...
![Code Modernization Phases](https://cookbook.openai.com/images/code-modernization-phases.png)
...
## [Phase 0 - Set up AGENTS and PLANS](https://cookbook.openai.com/examples/codex/code_modernization#phase-0---set-up-agents-and-plans)
...
### [Where Codex CLI helps](https://cookbook.openai.com/examples/codex/code_modernization#where-codex-cli-helps)
...
## [Phase 1 - Pick a pilot and create the first ExecPlan](https://cookbook.openai.com/examples/codex/code_modernization#phase-1---pick-a-pilot-and-create-the-first-execplan)
...
### [1.1 Choose pilot flow](https://cookbook.openai.com/examples/codex/code_modernization#11-choose-pilot-flow)

If you don’t have a flow in mind to pilot with, you can ask Codex to propose. Example prompt from the repository root:

```
Look through this repository and propose one or two candidate pilot flows for modernization that are realistic but bounded.For each candidate, list:- COBOL programs and copybooks involved- JCL members involved- The business scenario in plain language- End with a clear recommendation for which flow we should use as the first pilot
```

In this case, we’ll choose a reporting flow as the pilot.

![Pilot Candidate Flow](https://cookbook.openai.com/images/pilot-candidate.png)

### [1.2 Ask Codex to create the pilot ExecPlan](https://cookbook.openai.com/examples/codex/code_modernization#12-ask-codex-to-create-the-pilot-execplan)

```
Create pilot_execplan.md following .agent/PLANS.md. Scope it to the daily reporting flow. The plan should cover four outcomes for this one flow:- Inventory and diagrams- Modernization Technical Report content- Target design and spec- Test plan for parityUse the ExecPlan skeleton and fill it in with concrete references to the actual COBOL and JCL files.
```
...
## [Phase 2 - Inventory and discovery](https://cookbook.openai.com/examples/codex/code_modernization#phase-2---inventory-and-discovery)
...
### [2.1 Ask Codex to draft the overview](https://cookbook.openai.com/examples/codex/code_modernization#21-ask-codex-to-draft-the-overview)

```
Create or update pilot_reporting_overview.md with two top-level sections: “Inventory for the pilot” and “Modernization Technical Report for the pilot”.Use pilot_execplan.md to identify the pilot flow. In the inventory section, include:1. The COBOL programs and copybooks involved, grouped as batch, online, and utilities if applicable2. The JCL jobs and steps that call these programs3. The data sets or tables they read and write4. A simple text diagram that shows the sequence of jobs and data flows In the modernization technical report section, describe:1. The business scenario for this flow in plain language2. Detailed behavior of each COBOL program in the flow3. The data model for the key files and tables, including field names and meanings4. Known technical risks such as date handling, rounding, special error codes, or tricky conditions
```
...
![Pilot Flow Diagram](https://cookbook.openai.com/images/pilot-flow-diagram.png)

### [2.2 Update the ExecPlan](https://cookbook.openai.com/examples/codex/code_modernization#22-update-the-execplan)

Once the overview exists, ask Codex to keep the plan aligned

```
Update pilot_execplan.md to reflect the new pilot_reporting_overview.md file.- In Progress, mark the inventory and MTR sections as drafted.- Add any notable findings to Surprises and discoveries and Decision log.- Keep the ExecPlan readable for someone new to the repo.
```
...
## [Phase 3 - Design, spec, and validation plan](https://cookbook.openai.com/examples/codex/code_modernization#phase-3---design-spec-and-validation-plan)
...
### [3.1 Target design document](https://cookbook.openai.com/examples/codex/code_modernization#31-target-design-document)
...
### [3.2 API specification](https://cookbook.openai.com/examples/codex/code_modernization#32-api-specification)
...
```
Using pilot_reporting_design.md, draft an OpenAPI file at modern/openapi/pilot.yaml that describes the external API for this pilot. Include:- Paths and operations for the main endpoints or admin hooks- Request and response schemas for each operation- Field types and constraints, aligning with the target data model
```

Example output:

![Pilot Yaml](https://cookbook.openai.com/images/pilot-yaml.png)

### [3.3 Validation and test plan](https://cookbook.openai.com/examples/codex/code_modernization#33-validation-and-test-plan)
...
```
Using pilot_reporting_validation.md, create an initial test file at modern/tests/pilot_parity_test.py. Include placeholder assertions and comments that reference the scenarios in the test plan, but do not assume the modern implementation is present yet.
```

### [3.4 Update the ExecPlan](https://cookbook.openai.com/examples/codex/code_modernization#34-update-the-execplan)

```
Update pilot_execplan.md so that Plan of work, Concrete steps, and Validation and acceptance explicitly reference:1. pilot_reporting_overview.md2. pilot_reporting_design.md3. pilot_reporting_validation.md4. modern/openapi/pilot.yaml5. modern/tests/pilot_parity_test.py
```
...
## [Phase 4 - Implement and compare](https://cookbook.openai.com/examples/codex/code_modernization#phase-4---implement-and-compare)
...
### [4.1 Generate a first draft of the modern code](https://cookbook.openai.com/examples/codex/code_modernization#41-generate-a-first-draft-of-the-modern-code)

```
Using pilot_reporting_design.md and the COBOL programs listed in pilot_reporting_overview.md, generate initial implementation code under modern/<stack>/pilot that:- Defines domain models and database entities for the key records and tables.- Implements the core business logic in service classes, preserving behavior from COBOL paragraphs.- Adds comments that reference the original COBOL paragraphs and copybooks.- Treat this as a first draft for engineers to review.
```

You can run this several times, focusing on different modules.

### [4.2 Wire up the parity tests](https://cookbook.openai.com/examples/codex/code_modernization#42-wire-up-the-parity-tests)

```
Extend modern/tests/pilot_parity_test.py so that it:- Invokes the legacy pilot flow using whatever wrapper or command we have for COBOL (for example a script that runs the JCL in a test harness).- Invokes the new implementation through its API or batch entry point.- Compares the outputs according to the “Parity and comparison strategy” in pilot_reporting_validation.md.
```

### [4.3 Document the parallel run steps](https://cookbook.openai.com/examples/codex/code_modernization#43-document-the-parallel-run-steps)

Rather than a separate parallel_run_pilot.md, reuse the validation doc:

```
Update the Parity and comparison strategy section in pilot_reporting_validation.md so that it includes a clear, ordered list of commands to:- Prepare or load the input data set- Run the COBOL pilot flow on that data- Run the modern pilot flow on the same data- Compare outputs and interpret the results- Include precise paths for outputs and a short description of what success looks like
```

### [4.4 (If needed) Use Codex for iterative fixes](https://cookbook.openai.com/examples/codex/code_modernization#44-if-needed-use-codex-for-iterative-fixes)
...
## [Phase 5 - Turn the pilot into a scalable motion](https://cookbook.openai.com/examples/codex/code_modernization#phase-5---turn-the-pilot-into-a-scalable-motion)
...
### [6.1 Template ExecPlan](https://cookbook.openai.com/examples/codex/code_modernization#61-template-execplan)

```
Look at the pilot files we created:1. pilot_reporting_overview.md2. pilot_reporting_design.md3. pilot_reporting_validation.md4. pilot_execplan.md Create template_modernization_execplan.md that a team can copy when modernizing another flow. It should:1. Follow .agent/PLANS.md2. Include placeholders for “Overview”, “Inventory”, “Modernization Technical Report”, “Target design”, and “Validation plan”3. Assume a similar pattern: overview doc, design doc, validation doc, OpenAPI spec, and tests.
```

### [6.2 How-to guide](https://cookbook.openai.com/examples/codex/code_modernization#62-how-to-guide)

```
Using the same pilot files, write how_to_use_codex_for_cobol_modernization.md that:1. Explains the phases at a high level (Pick a pilot, Inventory and discover, Design and spec, Implement and validate, Factory pattern).2. For each phase, lists where coding agents helps and points to the relevant files and example prompts.
```

---

## [Wrap up](https://cookbook.openai.com/examples/codex/code_modernization#wrap-up)
...
![Pilot Folder Structure](https://cookbook.openai.com/images/pilot-folder-structure.png)
